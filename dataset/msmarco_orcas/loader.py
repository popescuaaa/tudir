...
class OrcasSplit:
    ...
    def build_corpus(self) -> str:
        ...
        n_paths = os.listdir(self.corpus_files_path)
        if "queries.txt" in n_paths:
...
class QueryDocumentOrcasDataset(Dataset):
    def __init__(self, split: str, cache_location: str = "./dataset/msmarco_orcas/.cache") -> None:
        ...
        if not os.path.isfile(os.path.join(cache_location, f"{split}_doc_keys.pickle.gz")):
            self.doc_body = {}

            for doc_id in tqdm(self.orcas_dataset.document_query_dict.keys(),
                               total=len(self.orcas_dataset.document_query_dict),
                               desc="Addings queries to documents index"):
                doc_tuple = docs_store.get(doc_id)
                self.doc_body[doc_id] = doc_tuple.body

            with gzip.open(os.path.join(cache_location, f"{split}_doc_keys.pickle.gz"), 'wb') as f:
                pickle.dump(self.doc_body, f)
        else:
            with gzip.open(os.path.join(cache_location, f"{split}_doc_keys.pickle.gz"), 'rb') as f:
                self.doc_body = pickle.load(f)
